pro check_image_info_prior_to_seg, image_info_savefile
  ...
  for i=0, n_images-1 do begin
    ...
    if good eq 0 then begin
      ...
      if file_exists(meta) eq 1 then begin
    ...
    endelse
    ;check for cloudmask
    if file_exists(image_info[i].cloud_file) eq 0 then checkit[i].cloud_file = 0
    ;check for a tc image
    if file_exists(image_info[i].tc_file) eq 0 then checkit[i].tc_file = 0
  endfor
